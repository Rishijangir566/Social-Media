...
const Post = () => {
  ...
  const handleEmojiClick = (emojiData) => {
    ...
    setContent(
      content.substring(0, startPos) + emoji + content.substring(endPos)
    );
        ...
        <div className="bg-white/5 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/10 p-8 space-y-6 relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="relative w-full min-h-[130px] bg-white/5 border-2 border-white/20 rounded-2xl">
              ...
              {/* Hashtag Popup */}
              {showHashtagPopup && (
                <div className="absolute bottom-full mb-2 left-0 bg-white/10 backdrop-blur-lg p-2 rounded-lg text-white">
                  Searching for: <strong>#{hashtagQuery}</strong>
                </div>
              )}
              ...
              <button
                type="button"
                className="absolute bottom-3 right-3 text-white/60 hover:text-white/90 transition-colors"
